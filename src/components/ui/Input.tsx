...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ 
    ...
    useEffect(() => {
      // Only initialize Grammarly on text inputs that need it
      if (enableGrammarly && containerRef.current && (type === "text" || !type)) {
      ...
      <div className="mb-5" ref={containerRef}>
        {label && (
          <div className="flex justify-between items-center mb-2">
            ...
            {enableGrammarly && (type === "text" || !type) && (
        ...
        <div className="relative">
          <input
            type={type}
            className={cn(
              "flex h-10 w-full rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm shadow-sm transition-colors focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50",
              error && "border-red-500 focus:border-red-500 focus:ring-red-500",
              className
            )}
            ref={ref}
            id={id}
            data-gramm={enableGrammarly && (type === "text" || !type) ? "true" : "false"}
            {...props}
          />
          {error && (
